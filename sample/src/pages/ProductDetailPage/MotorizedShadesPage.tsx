...
const productData = {
  ...
  description: 'Experience the future of window treatments with our premium motorized Roman shades. Featuring whisper-quiet operation under 35dB, smart home integration with Alexa, Google Home, and Apple HomeKit, and a rechargeable battery that lasts up to 6 months on a single charge.',
  basePrice: 449.99,
  ...
  colors: [
    ...
    { id: 'gray', name: 'Light Gray', hex: '#9CA3AF', image: 'https://miaoda.feishu.cn/aily/api/v1/files/static/71c5a08726ba4857a88b02903542fc08_ve_miaoda' },
    { id: 'navy', name: 'Navy Blue', hex: '#2C3E50', image: 'https://miaoda.feishu.cn/aily/api/v1/files/static/190087f6844e4a6d9e9eecd6e38fdad3_ve_miaoda' },
  ],
  images: [
    { id: '1', url: 'https://miaoda.feishu.cn/aily/api/v1/files/static/32e10cbf3d664f158a4ef73292dda052_ve_miaoda', alt: 'Motorized shades in smart home' },
    { id: '2', url: 'https://miaoda.feishu.cn/aily/api/v1/files/static/58c5b1e0bea9442eb4d1ea59746e3978_ve_miaoda', alt: 'Remote control operation' },
    { id: '3', url: 'https://miaoda.feishu.cn/aily/api/v1/files/static/d6e513cd18d543ba880d3432113b8457_ve_miaoda', alt: 'Smartphone app control' },
    { id: '4', url: 'https://miaoda.feishu.cn/aily/api/v1/files/static/ef5c765748204a14b6ad03baebd04785_ve_miaoda', alt: 'Close-up of fabric texture' },
  ],
  liningOptions: [
    { id: 'light-filtering', name: 'Light Filtering', price: 0, description: 'Soft natural light with privacy' },
    { id: 'room-darkening', name: 'Room Darkening', price: 50, description: 'Blocks 90% of light' },
    { id: 'blackout', name: 'Blackout', price: 80, description: 'Complete light blockage, perfect for bedrooms' },
  ],
  controlOptions: [
    { id: 'remote', name: 'Remote Control', price: 0, description: 'Handheld remote included' },
    { id: 'wall-switch', name: 'Wall Switch', price: 35, description: 'Hardwired wall control' },
    { id: 'smart-hub', name: 'Smart Hub Bundle', price: 99, description: 'Hub for app and voice control' },
  ],
};
    ...
    if (isPlaying) {
      interval = setInterval(() => {
        setShadePosition(prev => {
          ...
          // Simulate decibel variation during operation (30-35dB)
      ...
      setDecibelLevel(28); // Ambient when stopped
    }
    ...
    <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Experience our sub-35dB motor technology</p>
        </div>
      ...
      {/* Window Animation */}
      <div className="relative bg-sky-200 rounded-xl overflow-hidden mb-4" style={{ height: '200px' }}>
        <div className="absolute inset-4 border-4 border-white rounded-lg overflow-hidden">
          ...
          >
            {/* Roman shade folds effect */}
            <div className="absolute inset-0 opacity-20" style={{
              backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 15px, rgba(0,0,0,0.1) 15px, rgba(0,0,0,0.1) 16px)'
            }} />
            {/* Bottom rail */}
              ...
              <div className="flex items-end gap-0.5 h-4">
                {[1, 2, 3, 4].map((i) => (
                  <div 
                    ...
                    style={{ 
                      height: isPlaying ? `${Math.min(100, (decibelLevel - 25) * (i * 0.8 + 2))}%` : '15%',
                      opacity: isPlaying ? 1 : 0.5
                    }}
                  />
                ))}
              </div>
              <span className="text-xs font-mono text-emerald-400">{decibelLevel.toFixed(1)} dB</span>
            </div>
          </div>
        </div>
      </div>
...
// === 难度评估仪表盘 ===
const DifficultyMeter = ({ level, time, message }: { level: 1 | 2 | 3; time: string; message: string }) => {
  const colors = {
    1: 'bg-emerald-500',
    2: 'bg-amber-500',
    3: 'bg-orange-500'
  ...
  return (
    <div className="bg-accent/30 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${colors[level]}`}>
          Level {level}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-primary font-medium italic">&quot;{message}&quot;</span>
      </div>
    </div>
  );
};
      ...
      <div className="flex border-b border-border">
        <button 
          ...
          className={`flex-1 py-3 px-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
            activeTab === 'video' ? 'bg-primary/5 text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
        ...
        <button 
          ...
          className={`flex-1 py-3 px-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
            activeTab === 'guide' ? 'bg-primary/5 text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
          ...
          <div className="space-y-3">
            <div className="relative aspect-video bg-slate-900 rounded-xl overflow-hidden flex items-center justify-center group cursor-pointer">
              ...
              <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                <span className="text-xs text-white/80">Motorized Shade Installation</span>
                <span className="text-xs text-white/60">1:24</span>
              </div>
            </div>
            <p className="text-sm text-muted-foreground">
              Watch our step-by-step guide for installing motorized shades. No electrician needed for battery-powered models.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <>
              ...
              <div className="absolute inset-8 border-4 border-slate-300 rounded bg-sky-50">
                ...
                <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 flex items-center">
                  ...
                  <span className="px-2 text-xs font-medium text-primary bg-white">Width</span>
                  <div className="flex-1 h-px bg-primary" />
                ...
                <div className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 flex flex-col items-center">
                  ...
                  <span className="py-1 text-xs font-medium text-primary bg-white writing-mode-vertical">Height</span>
                  <div className="w-px flex-1 bg-primary" />
                  <div className="w-2 h-2 border-r-2 border-b-2 border-primary rotate-45 -mb-1" />
                </div>
              </div>
              <div className="absolute bottom-2 left-2 right-2 text-center">
                <p className="text-xs text-muted-foreground">Measure inside the window frame</p>
              </div>
            </>
          ) : (
            <>
              {/* Outside mount diagram */}
              <div className="absolute inset-8">
                {/* Shade outside (larger) */}
                ...
                {/* Width arrows - wider than window */}
                <div className="absolute -left-2 -right-2 top-1/2 -translate-y-1/2 flex items-center">
                  ...
                  <div className="flex-1 h-px bg-primary" />
                  <span className="px-2 text-xs font-medium text-primary bg-white">Width + 4&quot;</span>
                  <div className="flex-1 h-px bg-primary" />
                  <div className="w-2 h-2 border-r-2 border-t-2 border-primary rotate-45" />
                </div>
                {/* Height arrows - taller than window */}
                <div className="absolute -top-2 -bottom-2 left-1/2 -translate-x-1/2 flex flex-col items-center">
                  ...
                  <div className="w-px flex-1 bg-primary" />
                  <span className="py-1 text-xs font-medium text-primary bg-white">Height + 3&quot;</span>
                  <div className="w-px flex-1 bg-primary" />
                  <div className="w-2 h-2 border-r-2 border-b-2 border-primary rotate-45" />
                </div>
              </div>
              <div className="absolute bottom-2 left-2 right-2 text-center">
                <p className="text-xs text-muted-foreground">Shade overlaps window frame by 2-3&quot;</p>
              </div>
            </>
          )}
        </div>
...
// === 尺寸计算器 ===
const SizeCalculator = ({ mountType }: { mountType: 'inside' | 'outside' }) => {
    ...
    <div className="bg-accent/30 rounded-xl p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input type="number" step="0.125" value={width} onChange={(e) => setWidth(e.target.value)} placeholder="36" className="h-10" />
        </div>
        <div>
          <Label className="text-xs mb-1.5 block">Window Height (&quot;)</Label>
          <Input type="number" step="0.125" value={height} onChange={(e) => setHeight(e.target.value)} placeholder="48" className="h-10" />
        </div>
      </div>
...
const MotorizedShadesPage: React.FC = () => {
  const navigate = useNavigate();
  const { addToCart } = useCart();
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [selectedColor, setSelectedColor] = useState(productData.colors[0]);
  const [selectedLining, setSelectedLining] = useState(productData.liningOptions[0]);
  const [selectedControl, setSelectedControl] = useState(productData.controlOptions[0]);
  ...
  const totalPrice = useMemo(() => {
    ...
    const unitPrice = (basePrice + liningPrice + controlPrice) * Math.max(sizeMultiplier, 0.7);
  ...
  return (
    <div className="w-full space-y-16 md:space-y-24 pb-16">
          ...
          <div className="space-y-5">
            <div className="space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                <Badge variant="outline" className="rounded-full text-xs font-medium text-purple-600 border-purple-200 bg-purple-50">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3 flex-wrap">
                {productData.colors.map((color) => (
                  <button key={color.id} onClick={() => setSelectedColor(color)} className={`relative w-12 h-12 rounded-xl border-2 transition-all ${selectedColor.id === color.id ? 'border-primary ring-2 ring-primary ring-offset-2' : 'border-border hover:border-muted-foreground'}`} style={{ backgroundColor: color.hex }} title={color.name}>
            ...
            {/* Dimensions with Measure Guide Toggle */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium">Dimensions (inches)</Label>
                <button onClick={() => setShowMeasureGuide(!showMeasureGuide)} className="text-xs text-primary hover:underline flex items-center gap-1">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={selectedLining.id} onValueChange={(val) => { const lining = productData.liningOptions.find(l => l.id === val); if (lining) setSelectedLining(lining); }}>
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={selectedControl.id} onValueChange={(val) => { const control = productData.controlOptions.find(c => c.id === val); if (control) setSelectedControl(control); }}>
                <div className="grid gap-3">
                  {productData.controlOptions.map((control) => (
                    <div key={control.id}>
                      <RadioGroupItem value={control.id} id={control.id} className="peer sr-only" />
                      <Label htmlFor={control.id} className="flex items-start gap-3 p-4 rounded-xl border-2 border-border cursor-pointer transition-all peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5 hover:border-muted-foreground">
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <span className="font-medium">{control.name}</span>
                            <span className="text-sm">{control.price > 0 ? `+$${control.price}` : 'Included'}</span>
                          </div>
            ...
            <div className="grid grid-cols-3 gap-3 py-2">
              <div className="flex flex-col items-center text-center gap-1.5 p-3 rounded-xl bg-purple-50 border border-purple-100">
                ...
                <span className="text-xs text-purple-700">Whisper<br/>Quiet</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-3 rounded-xl bg-blue-50 border border-blue-100">
                ...
                <span className="text-xs text-blue-700">6-Month<br/>Battery</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-3 rounded-xl bg-emerald-50 border border-emerald-100">
                ...
                <span className="text-xs text-emerald-700">5-Year<br/>Warranty</span>
              </div>
            </div>
      ...
      {/* SelectBlinds-style Professional Modules */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Difficulty Meter */}
          <div className="lg:col-span-1">
            <DifficultyMeter level={1} time="15-20 min" message="You got this!" />
          </div>
      ...
      {/* Silent Operation Demo */}
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6">
          <h2 className="font-serif text-2xl md:text-3xl font-semibold mb-2">Experience the Silence</h2>
          <p className="text-muted-foreground">Our advanced motor technology operates at under 35dB — quieter than a whisper</p>
        </div>
        <SilentOperationDemo />
      </section>

      {/* Smart Features Accordion */}
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <Accordion type="single" collapsible className="w-full" defaultValue="smart">
            ...
            <AccordionContent className="pb-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-xl bg-accent/30 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">Works with Alexa, Google Assistant, and Siri</p>
                </div>
                <div className="p-4 rounded-xl bg-accent/30 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">Automate opening/closing times</p>
                </div>
                <div className="p-4 rounded-xl bg-accent/30 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">Sync with local sunrise times</p>
                </div>
              </div>
            </AccordionContent>
          </AccordionItem>
          ...
          <AccordionItem value="battery" className="border-b border-border">
            ...
            <AccordionContent className="pb-6">
              <div className="space-y-4">
                <p className="text-muted-foreground">Our high-capacity lithium battery provides up to 6 months of use on a single charge, depending on usage frequency.</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="p-3 bg-emerald-50 rounded-lg text-center">
                    <p className="text-2xl font-bold text-emerald-600">6</p>
                    <p className="text-xs text-emerald-700">Months Battery Life</p>
                  </div>
                  <div className="p-3 bg-emerald-50 rounded-lg text-center">
          ...
          <AccordionItem value="specs" className="border-b border-border">
            <AccordionTrigger className="py-4 text-lg font-serif font-semibold hover:no-underline">Specifications</AccordionTrigger>
            <AccordionContent className="pb-6">
              <table className="w-full">
                <tbody className="divide-y divide-border">
                  <tr><td className="py-3 text-sm text-muted-foreground w-1/3">Motor Noise Level</td><td className="py-3 text-sm font-medium">&lt; 35dB</td></tr>
                  <tr><td className="py-3 text-sm text-muted-foreground">Battery Capacity</td><td className="py-3 text-sm font-medium">2600mAh Li-ion</td></tr>
                  <tr><td className="py-3 text-sm text-muted-foreground">Max Shade Weight</td><td className="py-3 text-sm font-medium">20 lbs</td></tr>
                  <tr><td className="py-3 text-sm text-muted-foreground">Smart Protocols</td><td className="py-3 text-sm font-medium">WiFi, Bluetooth 5.0</td></tr>
                  <tr><td className="py-3 text-sm text-muted-foreground">Warranty</td><td className="py-3 text-sm font-medium">5 Years on Motor</td></tr>
                </tbody>
              </table>
            </AccordionContent>
          </AccordionItem>
...
export default MotorizedShadesPage;
