...
const headerStyles = [
  { 
    ...
    fullness: '200%', 
    price: 0, 
    description: 'Classic single-pinch design with elegant, evenly spaced pleats. Timeless look that works with any decor.',
    image: 'https://miaoda.feishu.cn/aily/api/v1/files/static/d5da9e382c034ee7afb544a0e56cda09_ve_miaoda'
  },
  { 
    id: 'triple-pleat', 
    name: 'Triple Pleat', 
    fullness: '250%', 
    price: 35, 
    description: 'Three-finger pinch for maximum fullness. Creates a luxurious, formal appearance perfect for traditional spaces.',
    image: 'https://miaoda.feishu.cn/aily/api/v1/files/static/d5da9e382c034ee7afb544a0e56cda09_ve_miaoda'
  },
  { 
    id: 'tailor-pleat', 
    ...
    fullness: '180%', 
    price: 25, 
    description: 'Modern streamlined look with tailored precision. Clean lines for contemporary and minimalist interiors.',
    image: 'https://miaoda.feishu.cn/aily/api/v1/files/static/d5da9e382c034ee7afb544a0e56cda09_ve_miaoda'
  },
];
...
const CollapsibleSection: React.FC<CollapsibleSectionProps> = ({ title, subtitle, isOpen, onToggle, children }) => {
  return (
...
// ============================================
const ProductDetailLuxV7Page: React.FC = () => {
  ...
  // Accordion states
  const [openSections, setOpenSections] = useState<string[]>(['color', 'header', 'dimensions']);
            ...
            <div className="space-y-3">
              ...
              >
                <div className="pt-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-500">{productColors.length} colors available</span>
                    <button onClick={() => setIsSwatchDialogOpen(true)} className="text-sm text-slate-700 underline">Order Free Swatches</button>
                  </div>
                  <HorizontalScroll>
                    {productColors.map((color) => (
                      ...
                      >
                        {selectedColor.id === color.id && (
                          <CheckIcon className={`absolute inset-0 m-auto w-4 h-4 ${['#F8F6F1', '#E8E4DC', '#E8D5D0', '#FFFDD0'].includes(color.hex) ? 'text-slate-800' : 'text-white'}`} />
                        )}
                      </button>
                    ))}
                  </HorizontalScroll>
                    ...
                    <div className="space-y-3">
                      {headerStyles.map((style) => (
                        <div key={style.id}>
                          ...
                          <Label htmlFor={style.id} className="flex gap-4 p-3 border border-gray-200 cursor-pointer transition-all peer-data-[state=checked]:border-slate-800 peer-data-[state=checked]:bg-gray-50 hover:border-gray-300">
                            {/* Header Style Image */}
                            <div className="w-24 h-24 shrink-0 bg-gray-100 overflow-hidden">
                              <Image src={style.image} alt={style.name} className="w-full h-full object-cover" />
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-500 mb-2">{style.description}</p>
                              <span className="text-xs text-gray-400">Fullness: {style.fullness}</span>
                            </div>
                          </Label>
                        </div>
                      ))}
                ...
                <div className="pt-4 space-y-4">
                  ...
                  <div className="bg-gray-50 border border-gray-200 p-4">
                    ...
                    <div className="aspect-[16/9] bg-gray-100 border-2 border-dashed border-gray-300 flex flex-col items-center justify-center">
                      ...
                      <p className="text-xs text-gray-400 mt-1">Shows how to measure window for custom drapes</p>
                    </div>
                    <div className="mt-3 text-sm text-gray-600 space-y-1">
                      <p>1. Measure width at top, middle, bottom - use narrowest</p>
                      <p>2. Measure height at left, center, right - use longest</p>
                  ...
                  <div className="flex items-start gap-2 p-3 bg-blue-50 text-sm text-blue-800">
                    ...
                    <span>For a pair of panels, order 2 panels. Each panel will be made to the dimensions above. Standard pair coverage: 72-96 inches window width.</span>
                  </div>
                </div>
              </CollapsibleSection>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 pt-16 border-t border-gray-200">
            <div className="mb-8">
              ...
              <p className="text-gray-500">Everything you need to know about custom drapery</p>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="border border-gray-200 bg-white">
                  <button className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <span className="font-medium text-slate-900">How do I measure for custom drapes?</span>
                    <ChevronDownIcon className="w-5 h-5 text-gray-400" />
                  </button>
                  <div className="px-6 pb-4 text-gray-600 text-sm">
                    For width, measure the curtain rod length (not the window). For height, measure from the rod to where you want the curtains to end - floor-length should hover 1/2 inch above the floor, while puddle-length adds 2-4 inches for a luxurious pooling effect. Use our measurement guide for detailed instructions.
                  </div>
                </div>
                <div className="border border-gray-200 bg-white">
                  <button className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <span className="font-medium text-slate-900">What's the difference between pleat styles?</span>
                    <ChevronDownIcon className="w-5 h-5 text-gray-400" />
                  </button>
                  <div className="px-6 pb-4 text-gray-600 text-sm">
                    Pinch pleat offers a classic, formal look with sewn-in pleats that create structured folds. Grommet has metal rings for a modern, casual aesthetic with wide, even waves. Rod pocket slides directly onto the rod for a simple, gathered look. Each style affects how the fabric hangs and moves.
                  </div>
                </div>
                <div className="border border-gray-200 bg-white">
                  <button className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <span className="font-medium text-slate-900">Which lining should I choose?</span>
                    <ChevronDownIcon className="w-5 h-5 text-gray-400" />
                  </button>
                  <div className="px-6 pb-4 text-gray-600 text-sm">
                    Unlined is perfect for sheer, decorative panels. Standard lining adds weight and protects fabric from sun damage. Blackout lining blocks 99% of light for bedrooms and media rooms. Thermal lining provides insulation for energy efficiency and temperature control.
                  </div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="border border-gray-200 bg-white">
                  <button className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <span className="font-medium text-slate-900">How long until I receive my order?</span>
                    <ChevronDownIcon className="w-5 h-5 text-gray-400" />
                  </button>
                  <div className="px-6 pb-4 text-gray-600 text-sm">
                    Each drapery panel is handcrafted to your specifications. Production takes 7-10 business days, and shipping takes 3-5 business days. You'll receive tracking information as soon as your order ships. We offer free shipping on all orders over $200.
                  </div>
                </div>
                <div className="border border-gray-200 bg-white">
                  <button className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <span className="font-medium text-slate-900">Can I return custom drapery?</span>
                    <ChevronDownIcon className="w-5 h-5 text-gray-400" />
                  </button>
                  <div className="px-6 pb-4 text-gray-600 text-sm">
                    Because our drapery is custom-made to your exact measurements and specifications, we cannot accept returns or exchanges unless there's a manufacturing defect. We encourage ordering free fabric swatches first to ensure you're satisfied with the color and texture.
                  </div>
                </div>
                <div className="border border-gray-200 bg-white">
                  <button className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <span className="font-medium text-slate-900">Do you offer installation services?</span>
                    <ChevronDownIcon className="w-5 h-5 text-gray-400" />
                  </button>
                  <div className="px-6 pb-4 text-gray-600 text-sm">
                    While we don't provide direct installation services, our curtains are designed for easy DIY installation with included hardware and detailed instructions. We also partner with local installers in major metropolitan areas - contact us for recommendations in your area.
                  </div>
                </div>
              </div>
            </div>
...
export default ProductDetailLuxV7Page;
